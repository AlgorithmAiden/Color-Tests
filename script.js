...
let black = createColor(['hex', '#00000000'])
let white = createColor(['hex', '#ffffff00'])
let color = createColor([['saturation', 100], ['lightness', 50]])
let reverseColor = createColor([['saturation', 100], ['lightness', 50]])

let grid = []
let targetbs = 10
let gx = Math.round(canvas.width / targetbs)
let gy = Math.round(canvas.height / targetbs)
...
for (let x = 0; x < gx; x++) {
    grid[x] = []
    for (let y = 0; y < gy; y++)
        grid[x][y] = Math.abs((gx / 2 - x) * (gy / 2 - y)) * .1 + 5000
}
...
setInterval(() => {
    for (let x = 0; x < gx; x++) {
        for (let y = 0; y < gy; y++) {
            grid[x][y]++
            color.hue = grid[x][y]
            reverseColor.hue = (grid[x][y] + 50) % 100
            ctx.fillStyle = multiLerp([color, black, reverseColor], Math.abs(Math.sin(Date.now() / grid[x][y] / 1000))).hex
            ctx.fillRect(bx * x, by * y, bx, by)
        }
    }
}, 0)